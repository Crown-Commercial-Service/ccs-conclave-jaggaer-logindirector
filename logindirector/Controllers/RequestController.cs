    ...
    public class RequestController : Controller
    {
        ...
        internal RequestSessionModel getRequestModel()
        {
            ...
            if (Request != null && !string.IsNullOrWhiteSpace(Request.Host.Host) && !string.IsNullOrWhiteSpace(Request.Scheme) && !string.IsNullOrWhiteSpace(Request.Path) && !string.IsNullOrWhiteSpace(Request.Method))
            {
                ...
                if (Request.Host.Host.ToLower() == _configuration.GetValue<string>("SupportedSources:CatSource"))
                {
                    // CaT domain request
                ...
                {
                    // We already know it's an approved source request at this point, and there are only two approved sources, so this must be from the Jaegger domain
